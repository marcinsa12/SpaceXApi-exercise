(function(global) {
	...
	const moreDetails = Vue.component("detailer", {
		props: ["flight", "rocket"],
		template:
			'<div v-if="shouldShowModal()" v-on:click="disableModal($event)" class="modal">' +
	...
	var myApp = new Vue({
		...
		template:
			'<div class="results">' +
			"<h1>List of last {{flightsAmount}} SpaceX Launches - Past and upcoming</h1>" +
	...
	fetch(myApp.accessLaunchesURL)
		...
		.then(function(myJson) {
			const flightsToDisplay = myJson.filter(function(flight) {
				return flight.flight_number > myJson.length - myApp.flightsAmount;
			});
